...
if __name__ == "__main__":
    ...
    LOAD_PATH = "/media/nami/TE_GZ/Datenauswertung_DSA-Bilder/LSTM/"
    SAVE_PATH = "/media/nami/TE_GZ/Datenauswertung_DSA-Bilder/LSTM"
    ...
    #Load Checkpoints:
    checkpoint_frontal = torch.load(LOAD_PATH + "LSTM_frontal_149.pt")        
    checkpoint_lateral = torch.load(LOAD_PATH + "LSTM_lateral_149.pt")
    starting_epoch = checkpoint_frontal['epoch'] + 1 


    #Initialize LSTM for frontal images:
    model_frontal = LSTMModel(1024*1024, 50, 2, 1)
    model_frontal.load_state_dict(checkpoint_frontal['model_state_dict'])
    model_frontal.to(device)
    
    optimizer_frontal = torch.optim.Adam(model_frontal.parameters(), lr=0.0001)
    ...
    #Initialize LSTM for lateral images:
    model_lateral = LSTMModel(1024*1024, 50, 2, 1)
    model_lateral.load_state_dict(checkpoint_lateral['model_state_dict'])
    model_lateral.to(device)
  
    optimizer_lateral = torch.optim.Adam(model_lateral.parameters(), lr=0.0001)
    ...
    for epoch in range(starting_epoch, 300):
        model_frontal.train()
        model_lateral.train()
