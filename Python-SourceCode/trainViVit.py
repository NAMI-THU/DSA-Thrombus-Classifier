...
if __name__ == "__main__":
    ...
    PATH = "/media/nami/TE_GZ/Datenauswertung_DSA-Bilder/CrossFoldValidation/Models/Vivit/Vivit_v1/"
    ...
    for fold in range(2, 6):
        ...
        #Load Checkpoints:  
        if fold == 1:
            checkpoint_frontal = torch.load(PATH + "frontal_last_fold_1.pt")        
            checkpoint_lateral = torch.load(PATH + "lateral_last_fold_1.pt")
            checkpoint_best_mcc_frontal = torch.load(PATH + "frontal_best_mcc_fold_1.pt")        
            checkpoint_best_mcc_lateral = torch.load(PATH + "lateral_best_mcc_fold_1.pt")
            print(checkpoint_best_mcc_frontal['acc'])
            print(checkpoint_best_mcc_frontal['mcc'])
            print(checkpoint_best_mcc_lateral['acc'])
            print(checkpoint_best_mcc_lateral['mcc'])
        elif fold == 2:
        ...
        starting_epoch = checkpoint_frontal['epoch'] + 1 if fold <= 2 else 0
        
        
        #Initialize CNN for frontal images:
        model_frontal = vivit.ViViT(512, 32, 1, 62)
        if fold <= 2:
            model_frontal.load_state_dict(checkpoint_frontal['model_state_dict'])
        model_frontal.to(device1)

        
        optimizer_frontal = torch.optim.AdamW(model_frontal.parameters(), lr=0.00001, weight_decay=0.1) 
        scheduler_frontal = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_frontal, 'min', factor=0.1, patience=40, verbose=True)
        loss_function_frontal = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor(1))
         
        
        #Initialize CNN for lateral images:
        model_lateral = vivit.ViViT(512, 32, 1, 62)
        if fold <= 2:
            model_lateral.load_state_dict(checkpoint_lateral['model_state_dict'])
        model_lateral.to(device2)
    
        
        optimizer_lateral = torch.optim.AdamW(model_lateral.parameters(), lr=0.00001, weight_decay=0.1)
        scheduler_lateral = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_lateral, 'min', factor=0.1, patience=40, verbose=True)
        ...
        for epoch in range(starting_epoch, 800):
            model_frontal.train()
            model_lateral.train()
            ...
            for step, batch in enumerate(dataLoaderTrain):
                ...
                images_frontal = images_frontal.unsqueeze(dim=2)
                ...
                images_lateral = images_lateral.unsqueeze(dim=2)
