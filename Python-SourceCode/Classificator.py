...
class Classificator:
    ...
    def load_models(self):
        ...
        self.model_frontal = CnnLstmModel(512, 3, 1, True, device)
        # model_frontal = torch.nn.DataParallel(model_frontal)
        self.model_frontal.load_state_dict(checkpoint_frontal['model_state_dict'])
        self.model_frontal.to(device)
        ...
        self.model_lateral = CnnLstmModel(512, 3, 1, True, device)
        # model_lateral = torch.nn.DataParallel(model_lateral)
        self.model_lateral.load_state_dict(checkpoint_lateral['model_state_dict'])
        self.model_lateral.to(device)
    ...
    def do_classification(self, image_frontal, image_lateral):
        ...
        output_frontal = self.model_frontal(images_frontal)
        output_lateral = self.model_lateral(images_lateral)
