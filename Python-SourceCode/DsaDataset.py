...
class DsaDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_data = nibabel.load(os.path.join(self.dataPath, entry['filename'])).get_fdata(caching='unchanged', dtype=np.float32) * 0.062271062 # = 255/4095.0
        ...
        if entry['frontalAndLateralView'] == False:
            ...
            image_data2 = nibabel.load(os.path.join(self.dataPath, entry['filenameOtherView'])).get_fdata(caching='unchanged', dtype=np.float32) * 0.062271062 
