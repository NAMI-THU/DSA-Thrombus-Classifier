...
public static class AiServiceCommunication
{
    private static readonly HttpClient Client = new()
        { BaseAddress = new Uri($"http://{Configuration.AiServiceUrl}/"), Timeout = TimeSpan.FromMinutes(5) };

    public static async Task<ClassificationResponse> ClassifySequence(string modelFrontal, string modelLateral,
        string fileFrontal, string fileLateral)
    {
        var request = new ClassificationRequest {
            PathFrontal = fileFrontal, PathLateral = fileLateral, ModelFrontal = modelFrontal,
            ModelLateral = modelLateral
        };
        var response = await Post(request, "/AiService/Classification");

        var content = await response.Content.ReadAsStringAsync();
        return JsonConvert.DeserializeObject<ClassificationResponse>(content) ??
               throw new ExternalException("Failed to convert servers response");
    }
    ...
    public static async Task<ImageResponse> LoadImages(string imageFrontal, string imageLateral,
